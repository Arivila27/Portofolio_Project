            ...
            <table class="table table-borderless">
                ...
                <tbody>
                    <tr v-for="(beli,index) in Displayedkasbank" :key="beli._id">
                        <th>{{index + 1}}</th>
                        <th>{{beli.kasbank_kode}}</th>
                        <th>{{beli.kasbank_tanggal}}</th>
                        <th>{{beli.kasbank_item}}</th>
                        <td v-if="beli.kasbank_type === 'Debit'"><span class="label label-warning">{{beli.kasbank_type}}</span></td>
                        <td v-else-if="beli.kasbank_type === 'Kredit'"><span class="label label-success">{{beli.kasbank_type}}</span></td>
                        <td>{{beli.kasbank_debit}}</td>
                        <td>{{beli.kasbank_kredit}}</td>
                        <td>
                            <router-link :to="{name:'detail_kasbank',params:{id:beli._id}}">Lihat</router-link>
