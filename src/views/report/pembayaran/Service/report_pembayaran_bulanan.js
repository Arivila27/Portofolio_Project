...
export default {
    ...
    methods: {
        ...
        getdatapenagihan: function() {
            axios.get(this.$API + `/Tr_penagihan/type/domain/Y/`+ this.$store.getters.domain)
                .then((response) => {
                    // var data = []
    ...
    computed: {
        target(){
            var romla = new Date(this.tanggal);
            var bulan_romla = "0" + (romla.getMonth());
            var tahun_romla = romla.getFullYear();
            var terakhir_hasil = tahun_romla + '-' + bulan_romla;
            //var fixtangal = this.momentt(terakhir_hasil)
            return terakhir_hasil
        },
        FilterTarget: function() {
            return this.data_closing.filter((closing) => {
                    var a = closing.closing_tanggal.toLowerCase().includes(this.target.toLowerCase());
                    return a;
            })
        },
        
        hitungtarget: function() {
            var target = 0;
            for (var i = 0; i < this.FilterTarget.length; i++) {
                target += parseInt(this.FilterTarget[i].closing_tagihan.closing_target_akhir);
            }
            return target;
        },
